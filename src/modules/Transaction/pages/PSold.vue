<template>
  <div>
    <div
      class="border-y border-slate-100 bg-[#EBF0F5] dark:border-[#343540] dark:bg-[#141414]"
    >
      <div class="flex justify-between py-3 container">
        <div class="flex gap-4 items-center">
          <div
            @click="openBottomSheet(1)"
            class="py-[7px] bg-[#D7DBE0] dark:bg-[#343540] px-5 rounded-lg transition-300 cursor-pointer"
          >
            <p
              :class="
        ...
        <div
          @click="openBottomSheet(1)"
          class="rounded-lg bg-[#D7DBE0] dark:bg-[#343540] p-[5px] h-8 w-8 flex items-center justify-center shrink-0 transition-300 cursor-pointer relative"
        >
...
const getTopUpList = async (): Promise<void> => {
  try {
    loading.value = true;
    TopUpList.value = await mainStore
      .fetchTransactions()
      .catch((error) => {
        useHandleError();
        handleError(error);
